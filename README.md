...
workflow must preserve (`docs-port.md` §3.2 in the
[`vilan-lang/proposals`](https://github.com/vilan-lang/proposals) repo).
...
| `CNAME`, `404.html`, `robots.txt`, `sitemap.xml`, `sitemap-root.xml`, `favicon.ico`, this README, `.gitignore` | hand-owned | `404.html` is also the `/vilan/*` → `/docs/*` forwarder older editor builds deep-link (its sunset: below) |
...
## The sitemap index (N13)

`sitemap.xml` is a sitemap index — the only sitemap `robots.txt` names —
referencing the two real ones: `sitemap-root.xml`, hand-kept, listing the
pages the repo root serves (`/` and `/playground/`), and the book's
`docs/sitemap.xml`, generated by `docs.yml` from whatever mdBook actually
built. Resolved by convention 2026-08-20 (N13, archived in the
`vilan-lang/proposals` repo's tracker): one entry point for crawlers now
that the chrome makes the two
halves look like one site, while the generated half stays generated and
neither writer learns about the other. No `lastmod` in any of the three,
deliberately — the book rebuilds on a daily cron whose "commit only if it
